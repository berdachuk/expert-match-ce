...
public abstract class BaseIntegrationTest {
    ...
    static {
        ...
        PostgreSQLContainer<?> container = new PostgreSQLContainer<>(
                DockerImageName.parse("expertmatch-postgres-test:latest")
    ...
    static void configureProperties(DynamicPropertyRegistry registry) {
        ...
        registry.add("spring.flyway.schemas", () -> "expertmatch");
        registry.add("spring.flyway.default-schema", () -> "expertmatch");
    ...
    private static void initializeDatabase() throws Exception {
        try (Connection conn = DriverManager.getConnection(
            ...
            try (Statement stmt = conn.createStatement()) {
                ...
                try {
                    stmt.execute("DROP SCHEMA IF EXISTS expertmatch CASCADE;");
                    log.debug("Dropped existing expertmatch schema for clean state");
                } catch (Exception e) {
                ...
                // Create fresh schema
                stmt.execute("CREATE SCHEMA expertmatch;");
                log.debug("Created fresh expertmatch schema");
                ...
                try {
                    ...
                    stmt.execute("SET search_path = ag_catalog, \"$user\", public, expertmatch;");
                    // Drop graph if it exists, then create fresh one
                    try {
                        stmt.execute("SELECT * FROM ag_catalog.drop_graph('expertmatch_graph', true);");
                    } catch (Exception e) {
                        // Graph might not exist, that's fine
                    }
                    // create_graph returns void, use PERFORM or SELECT * FROM
                    stmt.execute("SELECT * FROM ag_catalog.create_graph('expertmatch_graph');");
                ...
                // Set search path back to expertmatch schema
                try {
                    stmt.execute("SET search_path = expertmatch, public;");
                } catch (Exception e) {
